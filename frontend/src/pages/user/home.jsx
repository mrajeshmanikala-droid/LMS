...
export default function Home() {
  ...
  const fetchData = async () => {
    try {
      ...
      // 1️⃣ Get all books
      const booksRes = await axios.get(`${Server_URL}api/books`);
      ...
      const categoriesArr = Object.keys(categoryMap).map(cat => ({
        category: cat,
        count: categoryMap[cat]
      }));

      // 2️⃣ New arrivals
      const newRes = await axios.get(`${Server_URL}api/books/new`);
      const newBooks = newRes.data?.books || [];
      ...
      setStats({
        ...
        totalActiveStudents: 1 // backend not implemented yet
      });
  ...
  return (
    <div className="library-homepage">
      ...
      {/* <section className="na-section">
        ...
        <div className="na-grid-container">
          {newArrivals.map((book, i) => (
            <div key={i} className="na-book-item">
              <h4>{book.title}</h4>
              <p>{book.author}</p>
            </div>
          ))}
      ...
      {/* <section className="hours-section">
        <FiClock /> Mon–Fri: 8AM–8PM
        <FiCalendar /> Exam Time: 7AM–11PM
      </section> */}
    </div>
  );
}
